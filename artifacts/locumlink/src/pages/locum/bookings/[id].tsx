...
import { useGetBooking, getGetBookingQueryKey, useCheckInBooking, useCompleteBooking, useSignContract, useSubmitRating, useRaiseDispute, useGetBookingRatings, getGetBookingRatingsQueryKey } from "@workspace/api-client-react";
...
export default function LocumBookingDetail() {
  ...
  const locumHasRated = existingRatings?.data?.some((r: any) => r.raterType === "locum") ?? false;
                ...
                <>
                  ...
                  {locumHasRated ? (
                    <div className="flex items-center justify-center gap-2 text-sm text-amber-700 font-medium p-3 bg-amber-50 rounded-lg border border-amber-200">
                      <Star className="h-4 w-4 fill-amber-500 text-amber-500" /> Clinic Rated — Thank you!
                    </div>
                  ) : (
                    <Dialog>
