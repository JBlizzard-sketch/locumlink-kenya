...
export default function LocumMessages() {
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Your booking conversations with clinics.</p>
        </div>
        {totalUnread > 0 && (
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 gap-3 text-center">
            ...
            <p className="text-sm text-muted-foreground/70">
              Once a clinic confirms your booking, you can message them directly from the booking page.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-2">
          {conversations.map((conv) => (
            <Link key={conv.bookingId} href={`/locum/bookings/${conv.bookingId}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="font-semibold text-sm truncate">
                            {conv.shift?.clinicName ?? "Unknown Clinic"}
                          </span>
                      ...
                      <p className={`text-sm truncate ${conv.unreadCount > 0 ? "font-medium text-foreground" : "text-muted-foreground"}`}>
                        {conv.latestMessage.isFromMe ? (
                          ...
                          <span className="text-muted-foreground text-xs mr-1">{conv.latestMessage.senderName.split(" ")[0]}:</span>
                        )}
